...
class FeatureInteractionGraph(Module):
    ...
    def forward(self, x):
        ...
        # print("x_state_reshaped:",x_state_reshaped.shape)

        # (n, num_in, h, w) --> (n, num_node, h, w)
        #                   --> (n, num_node, h*w)
        x_proj_reshaped = self.conv_proj(x).view(batch_size, self.num_n, -1)
        # print("x_proj_reshaped:",x_proj_reshaped.shape)
        ...
        # print("x_n_state:",x_n_state.shape)

        # (n, num_state, num_node) -> (n, num_node, num_state)
        #                          -> (n, num_state, num_node)
        x_n_rel = self.gcn(x_n_state)
        # print("x_n_rel:",x_n_rel.shape)

        # reverse projection: instance space -> pixel space
        # (n, num_state, num_node) x (n, num_node, h*w) --> (n, num_state, h*w)
        x_state_reshaped = torch.matmul(x_n_rel, x_rproj_reshaped)
        # print("x_state_reshaped1:",x_state_reshaped.shape)
        ...
        # print("fc_2:",self.fc_2(x_state).shape)
        # print("feature out:",out.shape)
